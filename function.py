...
class Excel(object):
    def __init__(self, da_path, file_name, cursor_user):
        ...
        self.old_path = self.path + '/' + self.filename
        self.file_path = self.path + '/' + str(file_name).split('.')[0]+'_new.'+str(file_name).split('.')[1]
        print(self.file_path)
        self.meg = MergeCell(self.old_path)
        ...
        with open(self.file_path, 'rb') as f:
            ...
            for sheet in sheets.keys():
                ...
                for column in excel_sheet_data:
                    column_field = column_field + '`' + str(column) + '`' + ' ' + 'varchar(255),'
                column_field = column_field[:-1]
                column_insert = column_field.replace(' varchar(255)', '')
                create_tab_sql = "create table if not exists `%s`(%s) DEFAULT CHARSET=utf8" % (
                self.filename.split('.')[0] + '/' + sheet, column_field)
                print(create_tab_sql)
                create_res = conn_exec(create_tab_sql, self.cursor_user)
                # 获取行数超过10000拼接insert
                in_val_row = ""
                ...
                if excel_sheet_data.shape[0] > 10000:
                    ...
                    for i in range(yu):
                        ...
                        self.filename.split('.')[0] + '/' + sheet, column_insert, st)
                        print(insert_sql)
                        res = conn_exec(insert_sql, self.cursor_user)
                else:
                    insert_sql = "insert into `%s` (%s)values %s" % (
                    self.filename.split('.')[0] + '/' + sheet, column_insert, in_val_row)
                    print(insert_sql)
                    res = conn_exec(insert_sql, self.cursor_user)
        return res
...
def gb18030(filename, tablename, cursor_user):
    ...
    print(table)
    create_res = conn_exec(table, cursor_user)
